...
pub const id = "react/style-prop-object";

const message = "Style prop value must be an object";
...
pub const Bindings = struct {
    ...
    literal_names: [capacity][]const u8 = undefined,
    literal_values: [capacity]ast.NodeIndex = undefined,
    literal_len: usize = 0,

    pub fn addLiteral(self: *Bindings, name: []const u8, value: ast.NodeIndex) void {
        for (self.literal_names[0..self.literal_len]) |existing| {
            if (std.mem.eql(u8, existing, name)) return;
        }
        if (self.literal_len >= capacity) return;
        self.literal_names[self.literal_len] = name;
        self.literal_values[self.literal_len] = value;
        self.literal_len += 1;
    }

    pub fn findLiteral(self: *const Bindings, name: []const u8) ?ast.NodeIndex {
        for (self.literal_names[0..self.literal_len], 0..) |existing, index| {
            if (std.mem.eql(u8, existing, name)) return self.literal_values[index];
        }
        return null;
    }
};

pub fn bindingsFromProgram(tree: *const ast.Tree, program: ast.Program) Bindings {
    ...
    for (tree.extra(program.body)) |statement_index| {
        switch (tree.data(statement_index)) {
            ...
            .variable_declaration => |declaration| scanVariableDeclaration(tree, declaration, &bindings),
            else => {},
        }
    }
...
pub fn checkVariableDeclarator(tree: *const ast.Tree, declarator: ast.VariableDeclarator, bindings: *Bindings) void {
    ...
    addLiteralBinding(tree, declarator, bindings);
}
...
pub fn checkJSXAttribute(
    ...
    if (!std.mem.eql(u8, name, "style")) return;
    ...
    if (isNonNullaryLiteral(tree, container.expression)) {
        try addDiagnostic(allocator, diagnostics, tree, index);
        return;
    }

    const identifier = identifierReferenceName(tree, container.expression) orelse return;
    if (bindings.findLiteral(identifier)) |literal_index| {
        if (isNonNullaryLiteral(tree, literal_index)) {
            try addDiagnostic(allocator, diagnostics, tree, container.expression);
        }
    }
}
...
pub fn checkCallExpression(
    ...
    for (props) |property_index| {
        ...
        if (!std.mem.eql(u8, key, "style")) continue;

        if (identifierReferenceName(tree, property.value)) |identifier| {
            if (bindings.findLiteral(identifier)) |literal_index| {
                if (isNonNullaryLiteral(tree, literal_index)) {
                    try addDiagnostic(allocator, diagnostics, tree, property.value);
                }
            }
            return;
        }

        if (isNonNullaryLiteral(tree, property.value)) {
            try addDiagnostic(allocator, diagnostics, tree, property.value);
        }
        return;
    }
...
fn scanImportDeclaration(tree: *const ast.Tree, declaration: ast.ImportDeclaration, bindings: *Bindings) void {
...
fn scanVariableDeclaration(tree: *const ast.Tree, declaration: ast.VariableDeclaration, bindings: *Bindings) void {
...
fn addLiteralBinding(tree: *const ast.Tree, declarator: ast.VariableDeclarator, bindings: *Bindings) void {
    const name = bindingIdentifierName(tree, declarator.id) orelse return;
    if (!isNonNullaryLiteral(tree, declarator.init)) return;
    bindings.addLiteral(name, declarator.init);
}

fn isCreateElementCall(tree: *const ast.Tree, call: ast.CallExpression, bindings: Bindings) bool {
...
fn isBareCreateElementDeclarator(tree: *const ast.Tree, declarator: ast.VariableDeclarator, pragma: []const u8) bool {
    ...
    for (tree.extra(pattern.properties)) |property_index| {
        ...
        const local = bindingIdentifierName(tree, unwrapAssignmentPattern(tree, property.value)) orelse continue;
...
fn isRequireReactCall(tree: *const ast.Tree, index: ast.NodeIndex, pragma: []const u8) bool {
    ...
    const callee = identifierReferenceName(tree, unwrapTransparent(tree, call.callee)) orelse return false;
...
fn isNonNullaryLiteral(tree: *const ast.Tree, index: ast.NodeIndex) bool {
