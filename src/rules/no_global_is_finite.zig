...
const Visitor = struct {
    ...
    pub fn enter_call_expression(
        ...
        if (isGlobalIsFiniteCall(ctx.tree, self.symbol_table, call.callee)) {
...
fn isGlobalIsFiniteCall(
    ...
    if (identifierReferenceName(tree, unwrapped)) |name| {
        return std.mem.eql(u8, name, "isFinite") and isUnresolvedReference(symbol_table, unwrapped);
    }
    ...
    if (!std.mem.eql(u8, property_name, "isFinite")) return false;
