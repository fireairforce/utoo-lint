...
import 'dumi/theme-default/slots/ColorSwitch/index.less';
...
export default function ColorSwitch() {
  const { default: defaultColor } = useSiteData().themeConfig.prefersColor;
  const intl = useIntl();
  const [, prefersColor = defaultColor, setPrefersColor] = usePrefersColor();
  const mode = prefersColor as ColorMode;
  const currentLabel = intl.formatMessage({ id: `header.color.mode.${mode}` });

  return (
    <span
      className="dumi-default-color-switch"
      data-dumi-tooltip={currentLabel}
      data-dumi-tooltip-bottom
    >
      {icons[mode]}
      <select
        aria-label={intl.locale === 'zh-CN' ? '颜色主题' : 'Color theme'}
        onChange={(event) => setPrefersColor(event.target.value as ColorMode)}
        onClick={(event) => event.stopPropagation()}
        value={mode}
      >
        {(['light', 'dark', 'auto'] as const).map((color) => (
          <option key={color} value={color}>
            {intl.formatMessage({ id: `header.color.mode.${color}` })}
          </option>
        ))}
      </select>
    </span>
